    ...
    <div class="com">
      <div class="t1">
        ...
        <div class="t2">
          <div
            class="t3"
            @touchstart.stop="isType2 = 6"
            @click="
              isType = 6
              drawer = true
            "
            :class="isType2 == 6 ? 'activeBg' : ''"
          >
            <div>
              <img v-if="icon6" class="icon1" :src="icon6" alt="" />
              <p>{{ tokenSymbolSave || $t('computingPower.t1') }}</p>
            </div>
            <div>
              <img class="icon2" src="../../assets/new/16.png" alt="" />
            </div>
          ...
          >
            <input :placeholder="$t('computingPower.t20')" v-model="hash2" />
            <p class="save" @click="rechargeSaveFn">
              {{ $t('computingPower.t0') }}
            </p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    drawerFn(item) {
      ...
      if (this.isType == 6) {
        this.tokenSymbolSave = item.symbol
      ...
      try {
        self.axios
          .post(self.api.recharge, {
            hash: self.symbol == 'TRON USDT' ? self.hashSet : hash,
            chainId: getChainId,
            tokenSymbol,
          })
          .then((res) => {
    ...
    async rechargeSaveFn() {
      ...
      self.axios
        .post(self.api.recharge, {
          hash: self.hash2,
          chainId: getChainId,
          tokenSymbol: self.tokenSymbolSave,
        })
        .then((res) => {
          console.log(res)
          if (res.code == 200) {
            ...
            $('.modal-body-success').html(
              res.msg || self.$t('computingPower.t25'),
            )
            $('.clickDialogIconedSuccess').click()

            self.tokenSymbolSave = ''
            self.hash2 = ''
            self.icon6 = ''

            self.getAccountsFn()
          } else {
            $('.modal-body-erro').html(res.msg || self.$t('computingPower.t26'))
            $('.clickDialogIconedDanger').click()
    ...
    apiFn() {
      ...
      hashArr.forEach((e) => {
        if (e.tokenSymbol != 'TRON USDT') {
          this.rechargeFn2(e.hash, e.tokenSymbol)
        }
      })
    },
    setHashArrFn(hash, symbol) {
      if (symbol == 'TRON USDT') {
        return
      }
      if (localStorage.getItem('hashArr') && hash) {
        let hashArr = JSON.parse(localStorage.getItem('hashArr'))
        hashArr.push({
          tokenSymbol: symbol,
          hash: hash,
    ...
    async rechargeFn2(hash, tokenSymbol) {
      var self = this

      var getChainId = -1
      if (Web3.givenProvider && hash && hash != 'TRON USDT') {
        let web3 = new Web3(Web3.givenProvider)
        getChainId = await web3.eth.getChainId()
      }
